...
"""
OPTIONAL CODE FEATURING TOGGLES FOR ADDITIONAL AND ADVANCED CALCULATIONS:

# Perform lease calculations with optional additional and advanced metrics.
def calculate_lease_metrics(pmt, rate, nper, initial_cost, include_additional=False, include_advanced=False):
    lease_metrics = 
    {
        "Present Value (PV)": present_value(pmt, rate, nper),
        "Number of Periods (NPER)": number_of_periods(pmt, rate, present_value(pmt, rate, nper)),
...
# Toggles for additional or advanced calculations (optionally)
